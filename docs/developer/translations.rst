...
taking into account any customization of the translations (more on
`defining-available-languages <#defining-available-languages>`__, `add
translations <#add-translations>`__ and `customizing translations
<#customizing-translations-for-a-specific-language>`__).
...
message identifiers, see `update-translations <#update-translations>`__
...
**Note**: Do not remove the content headers from the ``.po`` files as it
is needed during the build process.
